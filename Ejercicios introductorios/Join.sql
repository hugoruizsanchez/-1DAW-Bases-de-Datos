...
SELECT * from movie, movie_cast; -- Podemos visualizar dos tablas a la vez con el join

SELECT count(*)  from movie, movie_cast; -- Podemos operar dos tablas a la vez con el join 

SELECT * from movie, movie_cast where mov_time=act_id; -- Visualizar JOIN SELECT con CONDICIONALES* FROM tabla1, tabla2 WHERE campo1=campo2;

SELECT movie.movie_title, movie_cast.act_id from movie, movie_cast;

SELECT mov_title, gen_title, movie.mov_id, movie_genres.mov_id 
FROM movie, genres, movie_genres 
where movie.mov_id = movie_genres.mov_id;

-- El resultado se mostrará de forma cartesiana siempre y cuando no se impoonga una condición ON. 
