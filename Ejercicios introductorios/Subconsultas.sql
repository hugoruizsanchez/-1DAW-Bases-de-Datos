...
ALTER TABLE rating
 ...
 -- 1. Consulta SQL que permita encontrar las películas sin ninguna clasificación (rating). Devolver el título de la película
 -- (Ver las PELÍCULAS cuya ID NO esté indexada en la tabla de movie_genres como mov_id. 
 
SELECT mov_title, mov_id from movie where mov_id NOT IN (SELECT mov_id FROM rating);
SELECT mov_title, mov_id from movie where NOT mov_id IN  (SELECT mov_id FROM rating) ;
 
 -- 2. Consulta SQL para encontrar a los actores que interpretaron un papel en la película 'Annie Hall' y que devuelva todos los campos de la tabla de actores
 
 -- (EL nombre y los apellidos de los actores están en la tabla "actor", como la id de la pelicula no puede relacionarse directamente, necesitamos
 -- "trasbordar" la información haciendo dos subconsultas: la primera llamando a act_id, y la segunda de act_id a mov_id, donde acotaremos la búsqueda al título). 
 
 SELECT act_fname, act_lname, act_id from actor where act_id IN (SELECT act_id from movie_cast where mov_id IN (SELECT mov_id FROM movie where mov_title like "Annie Hall"));

-- 3. Consulta SQL para identificar al director de la película 'Eyes Wide Shut', debe devolver el nombre, apellido

SELECT dir_fname, dir_lname, dir_id from director where dir_id IN (SELECT dir_id from movie_direction where mov_id IN (SELECT mov_id from movie where mov_title like "Eyes Wide Shut"));

-- 4. Consulta que permita listar el título de las películas de las que no se conoce el nombre del críticoC

SELECT mov_title from movie where mov_id IN (SELECT mov_id from rating where rev_id IN (SELECT rev_id from reviewer where rev_name is null or rev_name = "" ));

-- 5 Consulta que permita obtener el listado de las películas (su títulos) dirigidas por el director cuyo nombre es Woody y apellido es Allen
...
-- 6  Consulta para determinar los años en los que hubo al menos una película que recibió una calificación (rating) de al menos tres estrellas. Ordenar el resultado en orden ascendente por año de película
...
-- 7. Consulta que permita determinar las películas (sólo el título) que no tienen ninguna calificación

SELECT mov_title from movie where mov_id IN (SELECT mov_id from rating where num_o_ratings = 0);

-- 8. Consulta que permita obtener el título en orden ascendente de las películas que han sido valoradas por un crítico.

SELECT mov_title from movie where mov_id IN (SELECT mov_id from rating) ORDER BY mov_title;

-- 9   Consulta que permita obtener los críticos (nombre e identificador) que han valorado la película 'American Beauty'

SELECT rev_name, rev_id from reviewer where rev_id IN (SELECT rev_id from rating where mov_id IN (SELECT mov_id from movie where mov_title like "American Beauty")); 