/*
Actividad 6.13. Definir un procedimiento que muestre el resultado de la multiplicación de dos
números enteros e incluir el tratamiento de errores que muestre el mensaje “Se excede el mayor
valor posible que puede almacenar un entero” en caso de que el resultado exceda el rango de
los números que se pueden representar con el tipo de dato INTEGER (2147483647) y continúe con la ejecución
del procedimiento https://dev.mysql.com/doc/connector-j/8.0/en/connector-j-reference-error-sqlstates.html
*/

USE bd_programacion; 

DELIMITER $$
DROP PROCEDURE IF EXISTS bd_programacion.multiplica_enterosb $$
CREATE PROCEDURE bd_programacion.multiplica_enterosb (IN valor1 int , IN valor2 int) 
LANGUAGE SQL DETERMINISTIC
BEGIN
	DECLARE resultado integer;
    DECLARE error boolean default false; 
	DECLARE CONTINUE HANDLER FOR 1264 SET error = true;
    ...
    SELECT resultado;
    ELSE
    SELECT "ERROR: VALOR FUERA DE RANGO";
    END IF;
...
CALL bd_programacion.multiplica_enterosb (899999, 939494);
...
El problema con el handler en este procedimiento almacenado es que está mal ubicado. El manejador de errores handler_fuera_rango se ha definido después de la sentencia SELECT que se ejecuta después del cálculo del resultado.

Para que el manejador de errores funcione correctamente, debe definirse antes de la sentencia que puede generar el error, que en este caso es la asignación de la variable resultado.
