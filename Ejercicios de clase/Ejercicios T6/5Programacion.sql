...
/*Ejercicio 1. Crear un trigger que se ejecute cuando se produce el borrado de un registro de la
tabla pedido, que elimine los registros correspondientes de las tablas en los que hay registros
relacionados con el pedido borrado.

DELIMITER $$
CREATE TRIGGER disparador_jardineria AFTER DELETE ON pedido FOR EACH ROW
BEGIN
DELETE FROM
END $$

Para crear un trigger que elimine los registros correspondientes de las tablas en las que hay registros relacionados con el pedido borrado, primero debes identificar las tablas que tienen una relación con la tabla "pedido" a través de claves foráneas. Una vez que hayas identificado las tablas relacionadas, puedes escribir una consulta DELETE para borrar los registros correspondientes en esas tablas.

Por ejemplo, si hay una tabla llamada "detalle_pedido" que almacena los detalles de cada pedido y está relacionada con la tabla "pedido" a través de una clave foránea, puedes escribir el siguiente código para eliminar los registros correspondientes en la tabla "detalle_pedido" cuando se elimina un registro en la tabla "pedido":

DELIMITER $$
CREATE TRIGGER eliminar_detalle_pedido
...
En este ejemplo, "OLD.id_pedido" hace referencia al valor de la columna "id_pedido" en la fila que se está eliminando de la tabla "pedido".

Debes adaptar este código a las tablas de tu base de datos que tienen una relación con la tabla "pedido" a través de claves foráneas. También debes asegurarte de que el nombre del trigger y de las tablas y columnas coincidan con los nombres de tu base de datos.
...
DELIMITER $$
CREATE TRIGGER eliminar_detalle_pedido
BEFORE DELETE ON pedido -- Antes de la eliminación de pedido...
FOR EACH ROW
BEGIN
DELETE FROM detalle_pedido WHERE id_pedido = OLD.id_pedido; -- Eliminar todas las dependencias del mismo, esto es, las dependencias que correspondan a la antigua id de pedido, borrada. 
END$$
DELIMITER ;

/* Ejercicio 2. Crear un trigger que se ejecute cuando se produce el borrado de un registro de la
tabla cliente, que elimine los registros correspondientes de las tablas en los que hay registros
relacionados con el cliente borrado. 

Para crear un trigger que elimine los registros correspondientes de las tablas en las que hay registros relacionados con el cliente borrado, primero debes identificar las tablas que tienen una relación con la tabla "cliente" a través de claves foráneas. Una vez que hayas identificado las tablas relacionadas, puedes escribir una consulta DELETE para borrar los registros correspondientes en esas tablas.

Por ejemplo, si hay una tabla llamada "pedido" que almacena los pedidos de cada cliente y está relacionada con la tabla "cliente" a través de una clave foránea, puedes escribir el siguiente código para eliminar los registros correspondientes en la tabla "pedido" cuando se elimina un registro en la tabla "cliente":

DELIMITER $$
CREATE TRIGGER eliminar_pedidos
...
En este ejemplo, "OLD.id_cliente" hace referencia al valor de la columna "id_cliente" en la fila que se está eliminando de la tabla "cliente".

Debes adaptar este código a las tablas de tu base de datos que tienen una relación con la tabla "cliente" a través de claves foráneas. También debes asegurarte de que el nombre del trigger y de las tablas y columnas coincidan con los nombres de tu base de datos.
...
FOR EACH ROW
BEGIN
DELETE FROM pedido WHERE codigo_cliente = OLD.codigo_cliente;
DELETE FROM pàgo WHERE codigo_cliente = OLD.codigo_cliente;
END$$
DELIMITER ;
