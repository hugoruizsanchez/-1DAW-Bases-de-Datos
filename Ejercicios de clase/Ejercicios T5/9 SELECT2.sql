...
INSERT INTO producto VALUES ('FR-103','Olea-Olivos','Frutales','10/12','Frutales Talavera S.A','Existen dos 	 sobre el origen del olivo, una que postula que proviene de las costas de Siria, Líbano e Israel y otra que considera que lo considera originario de Asia menor. La llegada a Europa probablemente tuvo lugar de mano de los Fenicios, en transito por Chipre, Creta, e Islas del Mar Egeo, pasando a Grecia y más tarde a Italia. Los primeros indicios de la presencia del olivo en las costas mediterráneas españolas coinciden con el dominio romano, aunque fueron posteriormente los árabes los que impulsaron su cultivo en Andalucía, convirtiendo a España en el primer país productor de aceite de oliva a nivel mundial.',50,25,20);
...
-- -----------------------------------------------------------------------------
	...
	/*
    1. Obtener para las ciudades en las que hay oficinas el código de oficina y la ciudad.
    */
	
    SELECT oficina.codigo_oficina, ciudad
    from empleado
    JOIN oficina ON empleado.codigo_oficina = oficina.codigo_oficina
    GROUP BY ciudad;
    
    /*
    2. Obtener cuántos empleados hay en la compañía
    */
   ...
   SELECT COUNT(*) from empleado;
     ...
     /*
    3. Consulta que muestre cuántos clientes tiene cada país.
    */
	...
	/*
    4. Obtener el importe medio del pago en el año 2009.
    */
    
    SELECT AVG(total) as "LA MEDIA LA MENDIA" FROM pago where fecha_pago like "%2009%"; 
	SELECT AVG(total) as "LA MEDIA LA MENDIA" FROM pago where YEAR (fecha_pago) = 2009; 
    
    /*
5. Consulta que permita obtener informe de los pedidos que hay por cada estado,
mostrados de mayor a menor número
    */
    
    SELECT estado, codigo_pedido from pedido
    ORDER BY codigo_pedido  ;
    
    SELECT COUNT(*) AS num_pedidos, estado FROM pedido GROUP BY estado ORDER BY num_pedidos;
    ...
    */
    ...
    /*
    7. Consulta que permita obtener el nombre del cliente con el mayor límite de crédito
    */ 
  ...
  WHERE limite_credito = (SELECT MAX(limite_credito) FROM cliente);
    ...
    /*
    8. Obtener el nombre, apellidos y cargo de los empleados que no tienen ningún cliente
    asociado (no representan a ningún cliente).   
    */
...
/*
9. Obtener un listado con el nombre de cada cliente y el nombre y apellidos de su
...
/*
   10. Obtener la ciudad y el teléfono de las oficinas de Estados Unidos
*/

SELECT ciudad, telefono from oficina where pais = "EEUU";
/*
11.  Obtener el nombre, apellidos, cargo e email del jefe de la empresa
*/ 
    
SELECT nombre, apellido1, apellido2, email from empleado where puesto =  "Director General";

SELECT nombre, apellido1 , apellido2, email from empleado where codigo_jefe is null;

/*
13
Obtener el número de clientes que tiene cada país, ordenado por número de
clientes de mayor a menor número
*/ 

	SELECT COUNT(*) as num_clientes, Pais from cliente GROUP BY pais ORDER BY num_clientes desc;
    
/* 14.Mostrar el nombre de los clientes que no tienen pagos registrados, indicando
también el nombre y apellidos de su representante de ventas.
*/
...
/* 15. Obtener el listado de productos que han facturado más de 3000 euros. Para cada
producto se debe mostrar el nombre del producto, unidades vendidas, total facturado y total
facturado con impuestos (21% IVA) */

SELECT nombre, cantidad, SUM(cantidad*precio_unidad) as total_facturado, SUM(cantidad*precio_unidad)*1.21 as total_facturado_iva  
from producto 
JOIN detalle_pedido ON detalle_pedido.codigo_producto = producto.codigo_producto
HAVING total_facturado >3000;
 
 /* 16. Obtener la dirección de las oficinas que tienen clientes en la ciudad de Madrid */ 
...
/* 17. Obtener el nombre del producto que se vende más caro */ 
SELECT nombre
FROM producto
WHERE precio_venta = (SELECT MAX(precio_venta) FROM producto);

/* 18 Obtener los clientes (código de cliente, nombre del cliente, empleado responsable
de ventas, el límite de crédito y el total de pagos realizados) cuyo límite de crédito sea menor
que los pagos que haya realizado
